...
function checkCollisions() {
  ...
  for (let i = 0; i < blocks.length; i++) {
    if (
      ballCurrentCoords[0] > blocks[i].bottomLeft[0] &&
      ballCurrentCoords[0] < blocks[i].bottomRight[0] &&
      ballCurrentCoords[1] + ballDiameter > blocks[i].bottomLeft[1] &&
      ballCurrentCoords[1] < blocks[i].topLeft[1]
    ) {
  ...
  //player
  if (
    ballCurrentCoords[0] > playerCurrentCoords[0] &&
    ballCurrentCoords[0] < playerCurrentCoords[0] + playerWidth &&
    ballCurrentCoords[1] > playerCurrentCoords[1] &&
    ballCurrentCoords[1] < playerCurrentCoords[1] + playerHeight
  ) {
    redirect();
  }
  //gameover
...
function closePopup() {
  ...
  const bgBlur = document.querySelector(".container");
